...
class CrdcDH extends TestRunner implements Comparator<List<XSSFCell>>{
	...
	//			}
	//
	//			// Skip the first column ('type') in TSV
	//			List<List<String>> expectedRows = new ArrayList<>();
	//			for (int i = 1; i < lines.size(); i++) {
	//				String[] fields = lines.get(i).split("\t", -1); // retain empty fields
	//				List<String> trimmed = new ArrayList<>();
	//				for (int j = 1; j < fields.length; j++) {
	//					// skip column 0 (type)
	//					trimmed.add(fields[j].trim());
	//				}
	//				expectedRows.add(trimmed);
	//			}
	//
	//			// Get actual rows from UI
	//			List<WebElement> actualRowElements = Utils.getListWebElements('CRDC/DataSubmissions/Validation/Table-Row');
	//
	//			if (actualRowElements.size() != expectedRows.size()) {
	//				KeywordUtil.markFailed("Row count mismatch: UI has " + actualRowElements.size() +
	//						" rows, but file has " + expectedRows.size());
	//			}
	//
	...
	//					if (j == 0 || j == 2) continue; // Skip first and Status columns
	//
	...
	//
	//					try {
	//						WebElement span = cell.findElement(org.openqa.selenium.By.tagName("span"));
	...
	//					} catch (Exception e) {
	//						actual = cell.getText().trim(); // fallback if no span or aria-label
	//					}
	//
	...
	//					KeywordUtil.markFailed("Column count mismatch on row " + (i + 1) +
	//							": Expected " + expectedValues.size() + ", Found " + actualValues.size());
	//					continue;
	//				}
	//
	...
	//								": Expected to find '" + expected + "' in '" + actual + "'");
	//					}
	//				}
	//			}
	//
	//			KeywordUtil.logInfo("Finished verifying-- Data View tab in UI matches the contents of the TSV (ignoring type and Status columns).");
	//		} catch (Exception e) {
