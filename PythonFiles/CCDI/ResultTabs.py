...
if result_tab in ("TsvDataParticipants"):
...
elif result_tab in ("TsvDataStudies"):
    # Get participants query from excel
    studies_query = Utils.get_value_from_excel('StudiesTab')
    print(f'This is Studies query fitched from input excel:\n{studies_query}')
    # Executing query with dataframe and storing result
    ...
    # Print output data
    print(f'Studies data successfully written to: {output_excel}')

elif result_tab in ("TsvDataSamples"):
    # Get participants query from excel
    samples_query = Utils.get_value_from_excel('SamplesTab')
    print(f'This is Samples tab query fitched from input excel:\n{samples_query}')
    # Executing query with dataframe and storing result
    ...
    # Executing query with dataframe and storing result
    result_df_casefiles = Utils.df_run_query(files_query)   
    # Specify the output excel path
    output_excel = os.path.join(os.path.dirname(os.path.abspath(__file__)), Utils.get_output_file_path, Utils.get_value_from_excel('TsvExcel'))
    # Write the result DataFrame to an Excel file
    Utils.write_to_excel(output_excel, "TsvDataFiles", result_df_casefiles)
    # Print output data
